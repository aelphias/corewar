...
/*   Updated: 2021/01/20 19:47:08 by kcharlet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_args_type(t_car *car, unsigned char *arena, t_op *op)
{
	...
	if (car->is_type_code)
	{
		...
		if (op->args_amount >= 1)
			write_arg_type((arg_type & 0b11000000) >> 6, car, op, 1);
		if (op->args_amount >= 2)
			write_arg_type((arg_type & 0b00110000) >> 4, car, op, 2);
		if (op->args_amount >= 3)
			write_arg_type((arg_type & 0b00001100) >> 2, car, op, 3);
	}
	else
